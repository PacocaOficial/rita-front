...
export default function DeletePlan({ id, onDelete }: DeletePlanProps) {
    const { reset, clearErrors } = useForm<Required<{ id: number }>>({ id: id });
    const { token } = useAuth();
    ...
    const remove = async() => {
        ...
        setLoading(true)
        try {
            const response = await axios.delete(`${RITA_API_URL}/plans/manage`, {
                data: { id },
                headers: {
